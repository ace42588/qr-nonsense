...
export function addAlignmentPatterns(matrix: QRMatrix): QRMatrix {
  const size = matrix.length;
  const version = (size - 17) / 4;
  if (version === 1) return matrix; // Version 1 has no additional alignment patterns

  const positions = getAlignmentPatternPositions(version);

  function shouldDrawAlignmentPattern(x: number, y: number): boolean {
    const finderPatternPositions: Position[] = [
      { x: 0, y: 0 },
      { x: size - 7, y: 0 },
      { x: 0, y: size - 7 },
    ];
  ...
  function drawAlignmentPattern(centerX: number, centerY: number): void {
    for (let y = 0; y < 5; y++) {
      for (let x = 0; x < 5; x++) {
