...
import { generateQArt, QArtOptions } from "@/domain/qart";
import { Segment, QRMatrix, Codeword, VersionInfo, Input } from "@/types";
...
// The qart module imports from "../qr" which resolves to "@/domain/qr"
vi.mock("@/domain/qr/index", async () => {
  const actual = await vi.importActual("@/domain/qr/index") as any;
  return {
    ...actual,
