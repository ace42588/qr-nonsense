...
import { useInputDispatch, useParsedInputs } from "../state";
import {
  addBitFieldField,
  removeBitFieldField,
  updateBitFieldField,
...
function SortableField({ inputId, field, dispatch }) {
  ...
  const bitCount = field.mode === "max" ? bitsNeeded(field.max) : field.bitWidth || 1;

  const update = (key, value) => {
    dispatch(updateBitFieldField(inputId, field.id, key ? { [key]: value } : value));
  };

  return (
    <div ref={setNodeRef} className="flex items-center gap-2 mb-2 p-2 rounded border border-muted" style={style} {...attributes}>
      <span {...listeners} className="cursor-grab text-muted-foreground">☰</span>
      ...
      <Button variant="ghost" size="icon" onClick={() => dispatch(removeBitFieldField(inputId, field.id))}>
        ✕
      </Button>
    </div>
  );
...
function BitFieldEditor({ id, input }) {
  ...
  const handleAddField = () => {
    dispatch(addBitFieldField(id, { ...structuredClone(DEFAULT_FIELD), id: crypto.randomUUID() }));
  };
    ...
    <div className="space-y-4">
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={fields.map((f) => f.id)} strategy={verticalListSortingStrategy}>
          {fields.map((field) => (
            <SortableField key={field.id} inputId={id} field={field} dispatch={dispatch} />
          ))}
        </SortableContext>
      </DndContext>
...
function BitFieldValues({ id, input }) {
  ...
  const handleInputChange = (e, field) => {
    ...
    switch (type) {
      case "base10": newValue = Number(newValue); break;
      case "base16": newValue = parseInt(newValue, 16); break;
      case "string": newValue = encoder.encode(newValue); break;
      default: newValue = undefined;
    }
...
function BitFieldVisualizer({ id }) {
  ...
  return (
    <div className="mt-4">
      <div className="flex border rounded overflow-hidden text-white text-xs" style={{ height: 30, maxWidth: 600 }}>
      ...
      <div className="text-muted-foreground text-sm mt-2">{totalBits} bits total</div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm mt-2">
        {encodedBytes ? (
          <span><b>Encoded Bytes:</b> {encodedBytes}</span>
        ) : (
